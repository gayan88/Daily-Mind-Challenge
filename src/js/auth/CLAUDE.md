...
- `registeredUsers/{uid}` — uid is the Firebase Auth uid. `username` (permanent), `displayName`, `email` (optional, separate from the Auth login email), `isAdmin`, `isBanned`, `bannedReason`, `bannedDate`, `loginPoints`, `lastLoginDate` (gates the once-per-day login Points bonus only — see `utils/points.js#applyDailyLoginBonus()`), `currentStreak` + `lastStreakDate` (the Streak, Phase 4 — gameplay-based, **not** login-based, despite `currentStreak` having originally been login-driven before Phase 4; see `src/js/progression/streak-service.js` for the full rationale and the one-time transition logic), `xp` (global XP, Phase 3 — see `src/js/progression/xp-service.js`), `lastPerfectDayDate` (guards Phase 3's one-time-per-day "perfect day" XP bonus). See `docs/progression-gamification-roadmap.md` for the full phase plan.
