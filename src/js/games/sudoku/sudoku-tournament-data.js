 ...
 * unlike Wordle Tournament, a Sudoku Tournament run's total is split across many per-puzzle
 * gameScores docs plus this bonus doc, so there's no single doc whose own `score` field is "the
 * tournament total" for these to add to. Callers (sudoku-page.js) track the running total
 * themselves and use these only for their `applied` flag, not their returned `newScore`.
 * Independent from each other -- see wordle-daily-data.js's equivalent pair for the full
 * rationale (two separate, stackable bonuses, not alternatives).
 */
