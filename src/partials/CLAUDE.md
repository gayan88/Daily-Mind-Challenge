...
- **`footer.html`** — links to About & Contact (`/about`), Privacy Policy (`/privacy-policy`), Terms of Service (`/terms-of-service`), and Cookie Consent Notice (`/cookie-consent`), plus the "Share with Friends" link (`#footer-share-fb`, wired up in `app.js#wireFooterShare()` via the shared `shareUrl()` helper -- native Web Share API first, `sharer.php` popup fallback on desktop). Always shares the home page (`${origin}/`) with a fixed promotional message (`FOOTER_SHARE_TEXT` in `app.js`), regardless of which page the footer is clicked from -- this is a site-wide "come join us" invite, not a per-page share, so a friend clicking it should never land on the sharer's own `/settings` or `/profile` page. Leaderboard/Profile/Settings links were deliberately removed from here -- they're already in the header nav, and the footer was kept focused on the About/legal pages (added ahead of requesting Google AdSense approval, which expects a visible privacy policy and a real way to contact the site) plus the share link.
